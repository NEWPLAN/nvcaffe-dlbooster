...
template<typename DatumType>
shared_ptr<DatumType>& FPGAReader<DatumType>::DataCache::next_new() {
...
template<typename DatumType>
shared_ptr<DatumType>& FPGAReader<DatumType>::DataCache::next_cached(FPGAReader& reader) {
...
template<typename DatumType>
void FPGAReader<DatumType>::DataCache::just_cached() {
  just_cached_.store(true);
  cached_flags_[lwp_id()]->set();
}

template<typename DatumType>
bool FPGAReader<DatumType>::DataCache::check_memory() {
#ifdef __APPLE__
  return true;
#else
...
template<typename DatumType>
FPGAReader<DatumType>::CursorManager::CursorManager(db::DB* db, FPGAReader<DatumType>* reader,
...
template<typename DatumType>
FPGAReader<DatumType>::CursorManager::~CursorManager() {
  cursor_.reset();
  db_->Close();
}

template<typename DatumType>
void FPGAReader<DatumType>::CursorManager::next(shared_ptr<DatumType>& datum) {
  if (cached_all_) {
...
*/
template<typename DatumType>
void FPGAReader<DatumType>::CursorManager::rewind() {
...
template<>
void FPGAReader<Datum>::CursorManager::fetch(Datum* datum) {
...
template<>
void FPGAReader<AnnotatedDatum>::CursorManager::fetch(AnnotatedDatum* datum) {
  if (!cursor_->parse(datum)) {
    LOG(ERROR) << "Database cursor failed to parse Datum record";
  }
}

template class FPGAReader<Datum>;
template class FPGAReader<AnnotatedDatum>;
